"""
Baseline Model Comparison — 80 Epochs (AMPds)
==============================================
Runs baseline NILM models one at a time (useful when Colab times out).

Splits (1-min resolution, AMPds2):
  Train : 2013-11-21  (November)
  Val   : 2013-12-31  (December)
  Test  : 2012-08-23  (August)

Run ONE model per session:
    !python run_baseline_models_80epochs_ampds.py --model gru
    !python run_baseline_models_80epochs_ampds.py --model lstm
    !python run_baseline_models_80epochs_ampds.py --model resnet
    !python run_baseline_models_80epochs_ampds.py --model tcn
    !python run_baseline_models_80epochs_ampds.py --model transformer

Run ALL models in one go (original behaviour):
    !python run_baseline_models_80epochs_ampds.py --model all

Generate comparison graphs from saved results (after all models done):
    !python run_baseline_models_80epochs_ampds.py --plot

Each model saves its own JSON to results/baseline_80epochs_ampds/<model>.json
...
def load_data():
    print("Loading AMPds data...")
    splits = {}
    for split in ('train', 'val', 'test'):
        with open(f'data/AMPds/{split}.pkl', 'rb') as f:
...
def bar_chart(all_results, metric_key, metric_label, save_path):
    ...
    ax.set_title(f'{metric_label} per Appliance — Baseline Models (AMPds, 80 epochs)')
    ax.set_xticks(x)
...
def summary_chart(all_results, save_path):
    ...
    fig.suptitle('Summary — Baseline Models Averaged Over All Appliances (AMPds, 80 epochs)', fontsize=11)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  Saved → {save_path}")
...
def f1_heatmap(all_results, save_path):
    ...
    ax.set_title('F1 Score Heatmap — Baseline Models (AMPds, 80 epochs)')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  Saved → {save_path}")
...
def training_curves(model_key, curves, save_path):
    ...
    fig.suptitle(f'Training Curves — {MODEL_LABELS[model_key]} (AMPds, 80 epochs)', fontsize=12)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  Saved → {save_path}")
...
def epoch_metric_curves(model_key, epoch_metrics, save_path):
    ...
    for row, app in enumerate(APPLIANCES):
        ...
        for col, (mk_key, mk_label) in enumerate(metric_info):
            ...
            ax.set_ylabel(mk_label)
            ax.grid(True, alpha=0.3)
    fig.suptitle(f'Val Metrics per Epoch — {MODEL_LABELS[model_key]} (AMPds, 80 epochs)', fontsize=12)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  Saved → {save_path}")
...
def combined_epoch_metric_curves(all_epoch_metrics, save_path):
    ...
    for row, app in enumerate(APPLIANCES):
        for col, (mk_key, mk_label) in enumerate(metric_info):
            ...
            ax.grid(True, alpha=0.3)
    fig.suptitle('Val Metrics per Epoch — All Baseline Models (AMPds, 80 epochs)', fontsize=12)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"  Saved → {save_path}")
...
def print_table(all_results):
    ...
    for metric, label in [('f1', 'F1'), ('precision', 'Precision'), ('recall', 'Recall'), ('mae', 'MAE'), ('sae', 'SAE')]:
        print(f"\n{'='*80}")
        print(f"  {label} Comparison (AMPds)")
        print(f"{'='*80}")
...
SAVE_DIR = os.path.join('results', 'baseline_80epochs_ampds')
