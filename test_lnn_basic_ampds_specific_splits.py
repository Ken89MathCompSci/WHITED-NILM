"""
Basic LNN (LiquidNetworkModel) — AMPds specific splits
=======================================================
...
Usage:
    !python test_lnn_basic_ampds_specific_splits.py --appliance fridge
    !python test_lnn_basic_ampds_specific_splits.py --appliance oven
    !python test_lnn_basic_ampds_specific_splits.py --appliance "dish washer"
    !python test_lnn_basic_ampds_specific_splits.py --appliance "washer dryer"
    !python test_lnn_basic_ampds_specific_splits.py --plot
"""
...
def load_data():
    print("Loading AMPds data (specific splits)...")
    splits = {}
...
def plot_training_curves(results, save_dir):
    ...
    fig.suptitle('Training Curves — Basic LNN (AMPds)', fontsize=12)
    plt.tight_layout()
...
def plot_epoch_metrics(results, save_dir):
    ...
    fig.suptitle('Val Metrics per Epoch — Basic LNN (AMPds)', fontsize=12)
    plt.tight_layout()
...
def plot_bar_chart(results, save_dir):
    ...
    fig.suptitle('Final Test Metrics — Basic LNN (AMPds)', fontsize=12)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='Basic LNN — AMPds')
    ...
    for fp in ['data/AMPds/train.pkl',
               'data/AMPds/val.pkl',
               'data/AMPds/test.pkl']:
    ...
    if all_done:
        ...
        print('Run those, then: python test_lnn_basic_ampds_specific_splits.py --plot')
